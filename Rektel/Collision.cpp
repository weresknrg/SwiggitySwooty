...
void SAT::normalize(sf::Vector2f& vector)  //нормирование вектора (делим компоненты на длину)
{
...
float SAT::dot(sf::Vector2f& vector1, sf::Vector2f& vector2) // Скалярное произведение
{
	return vector1.x*vector2.x + vector1.y*vector2.y;
}

float SAT::distance(float minA, float maxA, float minB, float maxB) // расстояние, если + то пересечений нет
{
...
void  SAT::project(sf::Vector2f& axis, sf::VertexArray &_rectangle, float &min, float &max) // проекция одного вектора на другой
{
...
bool SAT::checkCollision(sf::Sprite &s1, sf::FloatRect &s2)
{
